...
function build_test_and_report() {
    ...
    git clone --recursive "$TESTSUITE_TESTSUITE_URL" --depth 1 --single-branch --branch "$branch" >& $PWD/git_clone.log || fatal_error "error cloning testsuite repository" $PWD/fulllog.txt
